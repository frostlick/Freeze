...
module Platform =
    ...
    let save text = async {
        ...
        if result.HasValue && result.Value then
            do! IO.File.WriteAllTextAsync(dlg.FileName, text) |> Async.AwaitTask
            return SaveSuccess
        else
            return SaveCanceled
        }
    ...
    let load () = async {
        ...
        let result = dlg.ShowDialog ()
        if result.HasValue && result.Value then
            let! contents = IO.File.ReadAllTextAsync(dlg.FileName) |> Async.AwaitTask
...
let designVm = ViewModel.designInstance (init () |> fst) (Platform.bindings ())
